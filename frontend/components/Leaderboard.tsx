            ...
            <tr key={entry.userId} className="group hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
              ...
              <td className="px-6 py-5">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-600 dark:to-slate-700 flex items-center justify-center font-bold text-slate-500 dark:text-slate-300">
                    {entry.email[0].toUpperCase()}
                  </div>
                  <span className="font-semibold text-slate-700 dark:text-slate-200 lowercase">{entry.email}***</span>
                </div>
              </td>
              <td className="px-6 py-5 text-right font-num font-bold text-slate-900 dark:text-slate-100">
                ${entry.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </td>
              <td className="px-6 py-5 text-right">
                <span className="rounded-full bg-blue-100 dark:bg-blue-900/40 px-3 py-1 text-xs font-bold text-blue-700 dark:text-blue-400 font-num">
                  {entry.exp} XP
                </span>
              </td>
            </tr>
